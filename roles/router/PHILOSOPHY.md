...
Moved from `fleet/agents/handler/router/` on 2026-06-27, as part of the actor/role/skill refactor (`~/.claude/taxonomy-refactor.md`). The router doc split into a **role** and a **skill**: the disposition → `roles/router/ROLE.md`; the dispatch how-to + scripts → `skills/dispatch/`.

**Note for the next editor:** this is the *full* original philosophy, kept whole. It covers both the router role (the over-instructing exclusion, mission-vs-envelope, mechanical-decisions) and the dispatch skill (the tmux testing-trace, defaults-unreliable, command-verification). The skill-mechanics editorial properly belongs with `skills/dispatch/PHILOSOPHY.md`; splitting this two ways is a known de-dup follow-up, not done in this move.
