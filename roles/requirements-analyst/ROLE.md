...
Your operators go in where you cannot follow, and they will hit what neither of you predicted. When an instruction turns out wrong on contact, the goal is the only thing they can reconcile against. So the goal is what must survive: not "add a history view to the TUI" but "be able to navigate past messages and see what happened" — the first is a guess at *how*, the second endures when the how is wrong. The goal carries the target *and* the reason, and the reason is what lets an operator choose rightly when the map runs out.
...
- **Bring a real fork as the tradeoffs that drive it + a question that reveals the SC's preference** — never "A or B?", never a recommendation.

## When

Before a mission is written — ahead of the scribe. The first role the handler takes on a new piece of work.
