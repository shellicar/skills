...
**Waiting is an action you execute, not a turn you hand back.** To wait, chain the pause and the read into one `ExecV3` call: `sleep <seconds>` then `tmux capture-pane -p -t <@window>`, joined with `&&`, with the `ExecV3` `timeout` set *above* the sleep (a bare `sleep 45` is SIGTERM'd at the 30s default — pass a `timeout` greater than the sleep, e.g. `sleep 100` with `timeout: 120000`). Sleep roughly the last completed turn's duration, then capture; if it's still mid-`thinking`, sleep again. What you must **never** do is end your response with "I'll wait, then read their pane" — that is the opposite of waiting: it hands the turn back to the SC and stops, so nothing gets read and the SC has to prod you to continue. The wait only happens if a `sleep`+`capture-pane` command actually runs. Go quiet and execute the wait; do not narrate it and yield.
...
**Hold each "we can do better at" to the standard.** This is the one craft check you enforce yourself, holding the handler to the post-mortem skill; the mission judgments stay the SC's. The standard: one short capability, not a fix, not a problem restated, no "and" or "not" bolting a second thing on — and plain enough that anyone could understand it on first read, with no mental gymnastics to work out what it means. Beware the soft bar: "it's good, that's plain language" is the judgement every planner reaches for too early, and it is still not simple enough. The real test is not "plain language" but "would a stranger get it instantly"; if you have to do any work to decode what they mean, it is not there yet. Push on every one, every time — not only the compound or fix-laden ones. The SC's rule is that the socratic "what do you really mean by that?" runs on all three, every retro; a clean-looking item is still abstract until the handler has said it plainly. Push **socratically**, never "that's wrong":
...
3. Then ask it to write it plainly as one short "we can do better at" — plain enough that anyone could understand it on first read, no jargon, no cleverness, no mental gymnastics to decode it. The handler will not have the communication skills loaded, so when it stays dressed up, tell it to load `communication-fundamentals` and `voice-stephen` and read `medium-documentation` for what plain looks like, then say each again. Keep pushing until it is plain.
