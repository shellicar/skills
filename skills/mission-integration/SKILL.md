...
7. **Re-ground the other worktrees.** The squash moved `main`, so every other in-flight worktree is now based on the old one and drifts further the longer it waits. Run `fleet/scripts/rebase-worktrees.mjs` to rebase each safe worktree branch onto the new `main` — see below.
