...
**Mission states** — beyond *active*, a mission can be **parked** (deliberately shelved by the SC — not active, not done; still recoverable, recorded with its *why* and an *un-park trigger*) or **post-mortem-owing** (delivered, owes its retrospective — which only the cast that was there can give, so its conv id is the asset). Each non-active state is moved off the planning surface into its own tmux session (`parked`, `post-mortem`) but stays anchored in the record by handler conv id, so the active surface shows only live work.
