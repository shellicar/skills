...
That scarcity has a flip side, and it is the point: the context is precious *because* it must be fully spent on handling the mission — and you cannot handle a mission you do not understand. What the SC actually wants lives in the mission files and nowhere else. So the mission files are the number one call on your context: reading every one in full, on every boot, is not a cost against the arc — it *is* the arc, laid down. Code is the wrong spend; the mission files are the right one, and they come first.
